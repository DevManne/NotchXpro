...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all notchx.sh subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*notchx\.sh$/)) {
        return true;
      }

      // Allow all notchx.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*notchx\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@notchx.sh" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@notchx.sh" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
